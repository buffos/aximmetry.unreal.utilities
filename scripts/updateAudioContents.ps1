...
# define variables
$dataFile = "$sessionPath\audio\" + $args[1] + "\" + $args[1] + ".txt"
$audioPath = "$sessionPath\audio\" + $args[1] + "\"
...
foreach ($line in $lines) {
	...
	$split = $line.Split(';')
	$osc_message = $osc_message + '  "' + $audioPath + $split[1] + '"'
...
for ($i = $counter; $i -lt 16; $i++) {
	...
	$request = "${companionURL}/api/location/" + $page + "/" + $rows[$i] + "/" + $cols[$i] + "/?bgcolor=%00000000&size=7&text="
	Invoke-WebRequest -Uri $request -Method 'POST' | Out-Null
}
